...
export default createStore({
  ...
  mutations: {
    ...
    addNewDroneID(state,payload){
      ...
      // console.log("payload=",payload) // ['3'] 【陣列形式】
      // console.log("payload[0]=",payload[0]) // '3' 【字串形式】
      const droneID = state.user.droneId
      for(var i=0; i<droneID.length; i++){
        if(droneID[i].id == payload[0]) return //發現已存在此id，退出addNewDroneID函式
      }
      droneID.push({id:payload[0]})
